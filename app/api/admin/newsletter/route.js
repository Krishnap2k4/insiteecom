    ...
    try {
        ...
        if (globalFilter) {
            matchQuery.$or = [
                ...
                { source: { $regex: globalFilter, $options: 'i' } },
        ...
        const pipeline = [
            ...
            { $skip: start },
            { $limit: size },
            { $project: { email: 1, name: 1, source: 1, status: 1, verifiedAt: 1, createdAt: 1, deletedAt: 1 } },
        ]
        const data = await SubscriberModel.aggregate(pipeline)
        const totalRowCount = await SubscriberModel.countDocuments(matchQuery)
