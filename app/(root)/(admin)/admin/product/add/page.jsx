...
import { ADMIN_DASHBOARD, ADMIN_PRODUCT_EDIT, ADMIN_PRODUCT_SHOW } from '@/routes/AdminPanelRoute'
...
/**
 * Add Product — Shopify-style sectioned form.
 *
 * Submission redirects to the edit page on success so the admin can
 * immediately add variants, see the auto-generated publicId, and
 * tweak inventory without backtracking.
 */
const AddProductPage = () => {
    ...
    // Module-2 extension fields tracked locally (not in shared zSchema).
    ...
    // Storefront card display fields — control how this product appears
    // on home page sections / shop listing cards. All optional.
    ...
    const onSubmit = async (values) => {
        ...
        try {
            ...
            const { data: response } = await axios.post('/api/product/create', payload)
            if (!response?.success) throw new Error(response?.message || 'Could not create product.')

            showToast('success', response.message)
            // Navigate to edit so admin can immediately add variants.
            if (response.data?.productId) {
                router.push(ADMIN_PRODUCT_EDIT(response.data.productId))
            } else {
        ...
        <div>
            ...
            <Form {...form}>
                <form onSubmit={form.handleSubmit(onSubmit)} className='space-y-5'>
                    ...
                    <Section title='Basic information' description='Core identity of the product. Used everywhere.'>
                        <div className='grid md:grid-cols-2 gap-5'>
                            ...
                            <FormField control={form.control} name='slug' render={({ field }) => (
                                <FormItem>
                                    ...
                                    <p className='text-xs text-gray-500'>Becomes part of the URL. A stable publicId is appended after saving.</p>
                                    <FormMessage />
                                </FormItem>
                            )} />
                        ...
                        <div className='grid md:grid-cols-3 gap-5'>
                            <FormField control={form.control} name='mrp' render={({ field }) => (
                                <FormItem>
                                    ...
                                    <FormControl><Input type='number' placeholder='List price' {...field} /></FormControl>
                                    <FormMessage />
                                </FormItem>
                            )} />
                            <FormField control={form.control} name='sellingPrice' render={({ field }) => (
                                <FormItem>
                                    ...
                                    <FormControl><Input type='number' placeholder='What the customer pays' {...field} /></FormControl>
                                    <FormMessage />
                                </FormItem>
                            )} />
                    ...
                    <Section title='Media' description='Images shown on listings and product page.'>
                    ...
                    <Section title='Description'>
                        <div className='space-y-5'>
                            <div>
                                ...
                                <Textarea
                                    rows={2}
                                    placeholder='One-line teaser shown on listing cards'
                    ...
                    <Section title='Tags & SEO' description='Discoverability — search, related products, social previews.'>
                        <div className='space-y-5'>
                            <div>
                                <Label className='mb-2 block flex items-center gap-2'><FiTag size={14} /> Tags</Label>
                                <TagsInput value={tags} onChange={setTags} />
                                <p className='text-xs text-gray-500 mt-1'>Comma or Enter to commit.</p>
                            </div>
                            <div className='grid md:grid-cols-2 gap-5'>
                                <div>
                                ...
                                <div className='md:col-span-2'>
                                    ...
                                    <Textarea rows={2} value={seoDescription} onChange={(e) => setSeoDescription(e.target.value)} placeholder='Short blurb for search engines and social previews' />
                                </div>
                                <div className='md:col-span-2'>
                    ...
                    <div className='rounded-lg bg-blue-50 border border-blue-200 p-4 flex items-start gap-3 text-sm text-blue-900'>
                        <FiInfo size={18} className='shrink-0 mt-0.5' />
                        <p>
                            After saving, you'll land on the product's edit page where you can add color / size variants and tune inventory. A default variant is auto-created so the product is purchasable immediately.
                        </p>
                    </div>
